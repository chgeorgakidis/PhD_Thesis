...
\subsubsubsection{FML-$k$NN workflow}
...
\subsubsubsection{Data pre-processing (stage 1)}
...
\begin{algorithm}[h!]
	...
	\KwIn{Datasets $R$, $S$, random vectors $\textbf{V}=\{\textbf{v}_1, ..., \textbf{v}_\alpha\}, \textbf{v}_1=\overrightarrow{0}$ and sampling threshold $MinThreshold$}
...
\subsubsubsection{Data partitioning and organization (stage 2)}
...
\subsubsubsection{$k$NN computation (stage 3)}
...
\subsubsubsection{Spark implementation}
\label{par:spark}
...
%More specifically, the \texttt{groupByKey} action was used in place of Flink's \texttt{groupBy}, which requires the dataset to be in a key-value pair format in order for the partitioning to properly work. Thus, in place of Flink's \texttt{flatMap}, the \texttt{flatMapToPair} Spark transformation was used, which maps each input element to one or more output items as key-value pairs. Flink's \texttt{groupReduce} transformation operates on grouped, properly partitioned datasets received as a whole and is able to produce one or more output values. The Spark transformation that achieves functionality similar to \texttt{groupReduce} is \texttt{mapPartitions}, which runs separately on each partition on the dataset, by receiving it and outputting the result as an iterable. Similarly to Flink, the level of parallelism of a \texttt{mapPartitions} transformation is automatically set equal to the number of partitions. For broadcasting the partitioning ranges, Spark offers a \texttt{broadcast} function, while for sorting a partition via a custom partitioner the \texttt{repartitionAndSortWithinPartitions} transformation is used. Finally, the saving of the results on HDFS is performed via the \texttt{saveAsTextFile} operation. The code of both Flink and Spark versions have been made publicly available, in order to motivate developers and researchers to make their own contributions.

\subsubsubsection{Cost analysis}
\label{par:cost}
